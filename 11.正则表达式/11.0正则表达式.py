...
要匹配变长的字符，在正则表达式中，用 * 表示任意个字符（包括0个），用 + 表示至少一个字符，用 ? 表示 0-1 个字符，用 {n} 表示 n 个字符，用 {n-m} 表示 n-m 个字符：
    例：\d{3}\s+\d{3,8}
    \d{3} 表示 3 个数字；\s+ 表示至少 1 个空格（也包括Tab等空白符）；\d{3,8} 表示 3-8 个数字，例如'1234567'
...
^ 表示行的开头，^\d 表示必须以数字开头
$ 表示行的结束，\d$ 表示必须以数字结束
py 可以匹配 'python'，但是加上 ^py$ 就变成了整行匹配，就只能匹配 'py' 了。


re 模块

有了准备只是，我们就可以在 Python 中使用正咋表达式。Python 提供 re 模块，包含所有正则表达式功能。
s = 'ABC\\-001'
s = r'ABC\-001'
...
>>> m.groups()
('19', '05', '30')
这个正则表达式可以直接识别合法的时间。但是有些时候，用正则表达式也无法做到完全验证。
对于非法日期，用正则还是识别不了，或者说写出来非常困难，这时候就需要程序配合识别了。
...
>>> ('102300', '')
由于 \d+ 采用贪婪匹配，直接把最后面的 0 全部匹配了，结果 0* 只能匹配空字符串了。
必须让 \d+ 采用非贪婪匹配（也就是尽可能少匹配），才能把后面的 0 匹配出来，加个 ? 就可以让 \d+ 采取非贪婪匹配

编译

当我们在 Python 中使用正则表达式时，re 模块会干两件事情：
